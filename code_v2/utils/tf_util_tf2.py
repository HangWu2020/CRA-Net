...
def dense (scope_name, inputs, output_num, is_training, reuse_mode = False, activation_fun = None,
           regularizer = tf.keras.regularizers.l2(1e-4), uniform_initializer = True, use_bn = False, bn_momentum = 0.9):
    with tf.variable_scope (scope_name, reuse=reuse_mode):
        if uniform_initializer == True:
            initializer = tf2.keras.initializers.GlorotUniform()
        else:
            initializer = tf2.keras.initializers.GlorotNormal()
        
        outputs = tf.layers.dense(inputs, output_num,kernel_initializer=initializer,kernel_regularizer=regularizer)
...
def conv1d (scope_name, inputs, output_channel, is_training, reuse_mode = False, activation_fun = None, 
            regularizer = tf.keras.regularizers.l2(1e-4), uniform_initializer = True, use_bn = False, bn_momentum = 0.9):
    with tf.variable_scope (scope_name, reuse=reuse_mode):
        if uniform_initializer == True:
            initializer = tf2.keras.initializers.GlorotUniform()
        ...
        outputs = tf.layers.conv1d(inputs, output_channel, kernel_size=1, kernel_initializer=initializer, kernel_regularizer=regularizer)
