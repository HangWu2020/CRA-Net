...
class RAN_1l_cuda ():
    ...
    def local_encoder (self, inputs, npts, cores, neighbor,nsample):
        with tf.variable_scope ('local_encoder', reuse=False):
            _, _, local, _ = sample_group(inputs, cores, radius=neighbor, min_point=nsample)
            local = tf.reshape(local, [cores,1024,3])
            raw_fts_0,max_fts_0 = sharedMLP_fullin ('ec_10',local ,[64,128])
            unpool_fts = tf.tile(max_fts_0, [1,1024,1])
            concat_fts = tf.concat([raw_fts_0, unpool_fts], axis=2)
            _,concat_fts = sharedMLP_fullin ('ec_11',concat_fts, [256,512])
            local_fts = tf.transpose(concat_fts, [1,0,2])
    ...
    def gcn_decoder(self, glb_fts, loc_fts, cores, nheads, bn, train):
        ...
        with tf.variable_scope ('decoder_fine', reuse=False):
            ...
            _,coarse_fts = sharedMLP ('coarse_feature',coarse_pts, coarse_npts,[64,128,256], bn, train)
...
class RAN_1l ():
    ...
    def local_encoder (self, inputs, npts, cores,ball_index_0,ball_size_0):
        with tf.variable_scope ('local_encoder', reuse=False):
            ...
            raw_fts_0,max_fts_0 = sharedMLP ('ec_10',cloud_exp_0, ball_size_0,[64,128])
            unpool_fts = point_unpool(max_fts_0, ball_size_0)
            concat_fts = tf.concat([raw_fts_0, unpool_fts], axis=2)
            _,concat_fts = sharedMLP ('ec_11',concat_fts, ball_size_0, [256,512])
